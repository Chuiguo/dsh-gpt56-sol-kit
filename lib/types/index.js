...
import { isMutatingTool, SUBAGENT_TOOLS } from "./tool-policy.js";
        ...
        switch (command.action) {
            ...
            case 'verify': {
                ...
                const toolResults = liveEvents.filter(event => event.type === 'tool/result').map(event => {
                    ...
                    if (event.data.error !== undefined)
                        result.errorCode = event.data.error.code;
                    if (event.data.meta !== undefined && typeof event.data.meta === 'object' && event.data.meta !== null)
                        result.meta = event.data.meta;
                    return result;
                });
