...
import { DEFAULT_CONFIG, MODE_IDS, isSolMode, normalizeConfig } from "./config.js";
import { isSolRoute } from "./model-match.js";
import { MODES, initialModeState, requiresConfirmation, resetModeState, resolveRequestOverrides, selectModeState } from "./modes.js";
import { solPromptSection } from "./prompt.js";
import { isMutatingTool } from "./tool-policy.js";
import { contextDecision } from "./context-policy.js";
import { budgetDecision, computeCost, pricesKnown } from "./cost.js";
import { parseSolCommand, renderBudget, renderCapabilities, renderStatus, renderWorkflow } from "./commands.js";
import { classifyTask } from "./task-profile.js";
import { createWorkflow, failureTransition, transition } from "./workflow.js";
import { resolveCapabilities } from "./capabilities.js";
import { evaluateBudget } from "./budget-v2.js";
import { resolveEffectivePolicy } from "./effective-policy.js";
import { assessVerification, collectVerificationEvidence } from "./verify.js";
import { classifyFailure, retryDecision } from "./preflight.js";
...
export function apply(ctx, config) {
    ...
    const pendingPersistence = new WeakSet();
    ...
    function restoreState(agent, state) {
        ...
        const migrated = version === 1
            ? { ...saved, schemaVersion: 2, consecutiveRequestErrors: 0, identicalRequestRetries: 0, taskStartedAtSeq: 0, providerFailures: [], workflow: { ...saved.workflow, maxFixRounds: runtimeConfig.maxFixRounds } }
            : saved;
    ...
    ctx.systemPrompt.section({
        ...
        text: (context) => {
            ...
            if (agent === undefined || !isSolAgent(agent))
                return '';
    ...
    ctx.on('agent/pre-step', async ({ agent, messages, turn }, next) => {
        ...
        if (state.mode === 'auto' && state.classifiedTurn !== turn) {
            ...
            applyRestriction(agent, state);
            const evidence = { kind: 'inspection', detail: `classified ${profile.scope} task` };
    ...
    ctx.tools.guard((execution) => {
        ...
        if (info.hardStop)
            return info.reason ?? 'V2 hard budget limit reached.';
        return undefined;
    });
    ...
    ctx.on('agent/request', async (payload, next) => {
        ...
        if (route !== undefined) {
            ...
            if (overrides.reasoningEffort === undefined)
                ...
                nextConfig.reasoningEffort = ReasoningEffortId(overrides.reasoningEffort);
            if (overrides.maxTokens === undefined)
                delete nextConfig.maxTokens;
            else
                nextConfig.maxTokens = overrides.maxTokens;
    ...
    ctx.on('session/event', (session, event) => {
        ...
        if (event.type === 'tool/result') {
            ...
            const evidence = event.data.error === undefined
                ? { kind: 'implementation', detail: `tool ${event.data.message.source.callId} completed`, passed: true }
                : { kind: 'failure', detail: `${failureClass}: ${event.data.error.code}`, passed: false };
    ...
    function persistStateForSession(session, state) {
        if (pendingPersistence.has(session))
            return;
    ...
    function handleSolCommand(invocation) {
        ...
        switch (command.action) {
            ...
            case 'phase':
                if (!isSol || state === undefined)
                    return { kind: 'error', text: 'Sol mode is inactive for the current model.' };
                return {
                    kind: 'success',
                    text: renderWorkflow({
                        phase: state.workflow.phase,
            ...
            case 'reset': {
                ...
                if (settings === undefined)
                    return { kind: 'error', text: 'No settings provider is mounted; reset is unavailable.' };
                void settings.replace(SETTINGS_NS, {}).catch((error) => {
            ...
            case 'verify': {
                ...
                const toolCalls = liveEvents.filter(event => event.type === 'tool/call').map(event => ({ callId: String(event.data.callId), name: event.data.name, arguments: event.data.arguments }));
                const toolResults = liveEvents.filter(event => event.type === 'tool/result').map(event => ({ callId: String(event.data.message.source.callId), ...(event.data.error === undefined ? {} : { errorCode: event.data.error.code }) }));
                const evidence = collectVerificationEvidence({ calls: toolCalls, results: toolResults, providerFailures: state.providerFailures, webTask: state.workflow.profile.scope === 'frontend' });
                const result = assessVerification(evidence);
                if (result.status === 'PASS') {
                    if (state.workflow.phase === 'verify')
                        state.workflow = transition(state.workflow, 'review', { kind: 'verification', detail: 'evidence assessment passed', passed: true });
                    if (state.workflow.phase === 'review')
                        state.workflow = transition(state.workflow, 'complete', { kind: 'review', detail: 'read-only evidence review passed', passed: true });
                    persistState(agent, state);
                    applyRestriction(agent, state);
                }
    ...
    function buildStatus(agent, route, state) {
        ...
        return {
            ...
            sessionCost: state === undefined ? null : computeCost({ inputTokens: state.inputTokens, cachedInputTokens: state.cachedInputTokens, outputTokens: state.outputTokens }, runtimeConfig),
