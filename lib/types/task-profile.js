 ...
 */
export function classifyTask(request, explicitMode) {
    ...
    const scope = readOnly && REVIEW_WORDS.test(trimmed)
        ? 'review'
        : readOnly
            ? 'diagnose'
                        ...
                        : DEEP_WORDS.test(trimmed)
                            ...
                            : modifying && !diagnostic
                                ...
                                : diagnostic
                                    ? 'diagnose'
                                    : 'answer';
    const mode = scope === 'frontend' ? 'frontend' : scope === 'review' ? 'review' : scope === 'deep-analysis' ? 'deep-analysis' : scope === 'modify' ? 'coding' : 'balanced';
...
export function allowsImplementation(scope) {
    return scope === 'modify' || scope === 'frontend';
}
/** Whether a mode must remain backend read-only. */
export function isProfileReadOnly(scope) {
