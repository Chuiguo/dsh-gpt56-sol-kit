...
function classifyTask(request, explicitMode) {
	...
	const scope = readOnly && REVIEW_WORDS.test(trimmed) ? "review" : readOnly ? "diagnose" : REVIEW_WORDS.test(trimmed) ? "review" : FRONTEND_WORDS.test(trimmed) && modifying ? "frontend" : DEEP_WORDS.test(trimmed) ? "deep-analysis" : modifying && (!diagnostic || STRONG_MODIFY.test(trimmed)) ? "modify" : diagnostic ? "diagnose" : "answer";
	return {
		scope,
...
function apply(ctx, config) {
	...
	ctx.on("agent/request-error", async ({ agent, failure }, next) => {
		...
		if (retryDecision(failureClass, state.identicalRequestRetries, state.consecutiveRequestErrors, runtimeConfig.maxIdenticalErrorRetries, runtimeConfig.maxConsecutiveToolErrors) === "retry") {
			persistState(agent, state);
			return { kind: "retry" };
		}
		...
		switch (command.action) {
			...
			case "verify": {
				...
				const toolResults = liveEvents.filter((event) => event.type === "tool/result").map((event) => ({
					...
					...event.data.error === void 0 ? {} : { errorCode: event.data.error.code }
				}));
