...
function normalizeConfig(input) {
	...
	const config = {
		...
		secondPass: raw.secondPass === void 0 ? DEFAULT_CONFIG.secondPass : expectBoolean(raw.secondPass, "secondPass"),
		useSubagents: raw.useSubagents === void 0 ? DEFAULT_CONFIG.useSubagents : expectBoolean(raw.useSubagents, "useSubagents"),
...
function classifyTask(request, explicitMode) {
	...
	const scope = REVIEW_WORDS.test(trimmed) ? "review" : FRONTEND_WORDS.test(trimmed) ? "frontend" : DEEP_WORDS.test(trimmed) ? "deep-analysis" : MODIFY_WORDS.test(trimmed) && !DIAGNOSE_WORDS.test(trimmed) ? "modify" : DIAGNOSE_WORDS.test(trimmed) ? "diagnose" : "answer";
	return {
		scope,
...
function createWorkflow(profile) {
	return {
		profile,
		phase: "inspect",
		steps: 0,
...
function canTransition(state, next, evidence) {
	...
	if (next === "fix" && state.fixRounds >= 2) return false;
	if (next === "verify" && state.phase === "fix" && state.fixRounds >= 2) return false;
...
function apply(ctx, config) {
	...
	function restoreState(agent, state) {
		...
		if (saved.schemaVersion !== 1 || !isSolMode(saved.mode) || saved.workflow === void 0 || typeof saved.workflow.phase !== "string") {
	...
	function switchMode(agent, state, mode) {
		...
		state.workflow = createWorkflow(classifyTask("", mode));
		state.warned = false;
		applyRestriction(agent, state);
		persistState(agent, state);
	...
	ctx.on("agent/request-error", async ({ agent, failure }, next) => {
		...
		const failureClass = classifyFailure({ message: JSON.stringify(failure) }, { command: "provider request" });
	...
	ctx.tools.guard((execution) => {
		...
		if (policy.readOnly && isMutatingTool(execution.name)) return `tool "${execution.name}" is disabled in ${policy.scope} scope`;
	...
	ctx.on("session/event", (session, event) => {
		...
		if (event.type === "tool/result") {
			...
			if (state.workflow.phase === "implement" && event.data.error === void 0) state.workflow = transition(state.workflow, "verify", evidence);
	...
	function handleSolCommand(invocation) {
		...
		switch (command.action) {
			...
			case "verify": {
				...
				const failures = toolResults.filter((event) => event.data.error !== void 0);
				const successfulResultIds = new Set(toolResults.filter((event) => event.data.error === void 0).map((event) => event.data.message.source.callId));
