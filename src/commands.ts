...
export type SolCommand =
  | { action: 'status' }
  | { action: 'mode'; name?: string; confirm: boolean }
  | { action: 'phase' }
  | { action: 'workflow'; reset: boolean }
  | { action: 'capabilities' }
  | { action: 'budget' }
  | { action: 'verify' }
  | { action: 'review' }
 ...
 */
export function parseSolCommand(rawInput: string): SolCommand {
  const tokens = rawInput.trim().split(/\s+/).filter(t => t.length > 0)
  if (tokens.length === 0) return { action: 'status' }
  const [verb, arg, flag] = tokens as [string, string | undefined, string | undefined]
  switch (verb) {
    case 'status':
...
export function renderWorkflow(w: WorkflowSnapshot): string {
...
export function renderCapabilities(c: CapabilitySnapshot): string {
