 ...
 */
export function contextDecision(input: ContextPolicyInput): ContextDecision {
...
const KEEP_KINDS: ReadonlySet<CompactionKind> = new Set(['goal', 'constraint', 'decision', 'file', 'todo'])
 ...
 */
export function keepForCompaction(items: readonly CompactionItem[]): CompactionItem[] {
 ...
 */
export function summarizeForCompaction(items: readonly CompactionItem[]): string {
  const kept = keepForCompaction(items)
  if (kept.length === 0) return ''
  const sections: Record<CompactionKind, string[]> = {
    goal: [], constraint: [], decision: [], file: [], todo: [], reasoning: [], other: [],
  }
  for (const item of kept) sections[item.kind].push(item.text)
  const blocks: string[] = []
  if (sections.goal.length > 0) blocks.push('Goal:\n' + sections.goal.map(t => `- ${t}`).join('\n'))
  if (sections.constraint.length > 0) blocks.push('Constraints:\n' + sections.constraint.map(t => `- ${t}`).join('\n'))
  if (sections.decision.length > 0) blocks.push('Key decisions:\n' + sections.decision.map(t => `- ${t}`).join('\n'))
  if (sections.file.length > 0) blocks.push('Files modified:\n' + sections.file.map(t => `- ${t}`).join('\n'))
  if (sections.todo.length > 0) blocks.push('Unfinished:\n' + sections.todo.map(t => `- ${t}`).join('\n'))
  return blocks.join('\n\n')
}
