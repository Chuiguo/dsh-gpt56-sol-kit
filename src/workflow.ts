...
export function createWorkflow(profile: TaskProfile): WorkflowState {
...
export function failureTransition(state: WorkflowState, detail: string): { next: 'fix' | 'blocked'; evidence: WorkflowEvidence } {
  if (state.phase === 'verify' && state.fixRounds < 2) return { next: 'fix', evidence: { kind: 'failure', detail, passed: false } }
